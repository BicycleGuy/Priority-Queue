...
class WP_PQ_API
{
    ...
    public static function get_cached_user(int $user_id): ?\WP_User
    {
        ...
        if (! array_key_exists($user_id, self::$user_cache)) {
            self::$user_cache[$user_id] = get_user_by('ID', $user_id);
        }
    ...
    public static function preload_users(array $user_ids): void
    {
        ...
        foreach ($user_ids as $id) {
            $id = (int) $id;
            if ($id > 0 && ! array_key_exists($id, self::$user_cache)) {
                $missing[] = $id;
            }
        }
        ...
        $users = get_users([
            'include' => array_values(array_unique($missing)),
            'fields' => 'all',
        ]);
        ...
        foreach ($missing as $id) {
            if (! array_key_exists($id, self::$user_cache)) {
                self::$user_cache[$id] = false;
            }
        }
    }

    /** @var array<int, array> */
    private static array $client_membership_cache = [];

    public static function get_cached_client_memberships(int $client_id): array
    {
        if ($client_id <= 0) {
            return [];
        }

        if (! array_key_exists($client_id, self::$client_membership_cache)) {
            self::$client_membership_cache[$client_id] = WP_PQ_DB::get_client_memberships($client_id);
        }

        return self::$client_membership_cache[$client_id];
    }
    ...
    public static function create_task(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (current_user_can(WP_PQ_Roles::CAP_APPROVE) && $requested_submitter_id > 0 && self::get_cached_user($requested_submitter_id)) {
            $submitter_id = $requested_submitter_id;
        }
        $owner_ids = self::sanitize_int_array($request->get_param('owner_ids'));
    ...
    public static function update_assignment(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $owner_ids = self::sanitize_int_array(json_decode((string) ($task['owner_ids'] ?? ''), true));
    ...
    private static function get_visible_tasks_for_request(?WP_REST_Request $request, int $user_id, bool $calendar_order = false): array
    {
        ...
        if (! user_can($user_id, WP_PQ_Roles::CAP_WORK)) {
            $access = [];
            $access[] = 'submitter_id = %d';
            $params[] = $user_id;
            $access[] = 'client_user_id = %d';
            $params[] = $user_id;
            $access[] = 'action_owner_id = %d';
            $params[] = $user_id;
            $access[] = $wpdb->prepare("JSON_CONTAINS(owner_ids, %s)", (string) $user_id);
            ...
            $admin_client_ids = array_values(array_unique(array_filter($admin_client_ids)));
            if (! empty($admin_client_ids)) {
                $access[] = 'client_id IN (' . implode(',', array_map('intval', $admin_client_ids)) . ')';
            }

            $job_bucket_ids = WP_PQ_DB::get_job_member_ids_for_user($user_id);
            if (! empty($job_bucket_ids)) {
                $access[] = 'billing_bucket_id IN (' . implode(',', array_map('intval', $job_bucket_ids)) . ')';
            }

            $where[] = '(' . implode(' OR ', $access) . ')';
        }

        $sql = "SELECT * FROM {$tasks_table} WHERE " . implode(' AND ', $where);
        $sql .= $calendar_order ? ' ORDER BY id DESC' : ' ORDER BY queue_position ASC, id DESC';
    ...
    private static function task_filter_clients(): array
    {
        ...
        return array_map(static function (array $row): array {
            $primary_contact = (int) ($row['primary_contact_user_id'] ?? 0) > 0
                ? self::get_cached_user((int) $row['primary_contact_user_id'])
                : null;
            return [
                'id' => (int) ($row['id'] ?? 0),
    ...
    private static function default_client_action_owner_id(int $client_id, int $primary_contact_user_id = 0): int
    {
        ...
        if ($primary_contact_user_id > 0 && in_array($primary_contact_user_id, $client_admin_ids, true) && self::get_cached_user($primary_contact_user_id)) {
            return $primary_contact_user_id;
        }

        foreach ($client_admin_ids as $admin_user_id) {
            if ($admin_user_id > 0 && self::get_cached_user($admin_user_id)) {
                return $admin_user_id;
            }
        }

        if ($primary_contact_user_id > 0 && self::get_cached_user($primary_contact_user_id)) {
            return $primary_contact_user_id;
        }
    ...
    public static function mark_notifications_read(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $table = $wpdb->prefix . 'pq_notifications';
        $user_id = get_current_user_id();
        $ids = self::sanitize_int_array($request->get_param('ids'));
    ...
    public static function create_invoice_draft(array $args, int $user_id = 0)
    {
        ...
        $task_ids = self::sanitize_int_array($args['task_ids'] ?? []);
        $entry_ids = self::sanitize_int_array($args['entry_ids'] ?? []);
        ...
        foreach ($rows as $row) {
            ...
            if ($task_id > 0) {
                ...
                self::emit_event($task_id, 'statement_batched');
            }
        }
    ...
    public static function create_work_log_batch(array $task_ids, string $notes = '', string $range_start = '', string $range_end = '', int $user_id = 0, array $snapshot_filters = [])
    {
        ...
        $task_ids = self::sanitize_int_array($task_ids);
        if (empty($task_ids)) {
    ...
    public static function preview_work_log_tasks(array $args): array
    {
        ...
        $job_ids = self::sanitize_int_array($args['job_ids'] ?? []);
    ...
    private static function emit_event(int $task_id, string $event_key): void
    {
        if ($task_id <= 0) {
            return;
        }
    ...
    private static function emit_assignment_event(int $task_id, int $previous_action_owner_id, int $action_owner_id): void
    {
        ...
        $recipient = self::get_cached_user($action_owner_id);
        if (! $recipient) {
            return;
        }
    ...
    private static function notify_mentions(int $task_id, array $task, string $body, int $author_id): void
    {
        ...
        foreach ($mentions as $handle) {
            ...
            $user = self::get_cached_user($mentioned_user_id);
    ...
    private static function client_notification_recipient_ids(array $task): array
    {
        ...
        $recipient_ids = [];
        foreach (self::get_cached_client_memberships($client_id) as $membership) {
    ...
    {
        if (! in_array($event_key, ['task_created', 'task_approved', 'task_clarification_requested', 'task_returned_to_work', 'task_delivered', 'task_archived'], true)) {
            return false;
        }
    ...
    private static function build_client_status_update_body(array $task, int $recipient_id, string $new_status): string
    {
        ...
        $where = [$wpdb->prepare("t.client_id = %d", $client_id)];
        $where[] = "t.status NOT IN ('done','archived')";

        if (! user_can($recipient_id, WP_PQ_Roles::CAP_VIEW_ALL) && ! user_can($recipient_id, WP_PQ_Roles::CAP_WORK)) {
            $access = [];
            $access[] = $wpdb->prepare("t.submitter_id = %d", $recipient_id);
            $access[] = $wpdb->prepare("t.client_user_id = %d", $recipient_id);
            $access[] = $wpdb->prepare("t.action_owner_id = %d", $recipient_id);
            $access[] = $wpdb->prepare("JSON_CONTAINS(t.owner_ids, %s)", (string) $recipient_id);

            $memberships = WP_PQ_DB::get_user_client_memberships($recipient_id);
            $is_admin = false;
            foreach ($memberships as $membership) {
                if ((int) ($membership['client_id'] ?? 0) === $client_id && (string) ($membership['role'] ?? '') === 'client_admin') {
                    $is_admin = true;
                    break;
                }
            }
            ...
            if (! $is_admin) {
                $job_bucket_ids = WP_PQ_DB::get_job_member_ids_for_user($recipient_id);
                if (! empty($job_bucket_ids)) {
                    $access[] = 't.billing_bucket_id IN (' . implode(',', array_map('intval', $job_bucket_ids)) . ')';
                }
                $where[] = '(' . implode(' OR ', $access) . ')';
            }
        }

        $visible_tasks = (array) $wpdb->get_results(
            "SELECT * FROM {$tasks_table} t WHERE " . implode(' AND ', $where) . " ORDER BY queue_position ASC, id DESC",
            ARRAY_A
        );
    ...
    private static function attach_author_names(array $rows): array
    {
        foreach ($rows as &$row) {
            ...
            $user = $author_id > 0 ? self::get_cached_user($author_id) : null;
    ...
    private static function build_event_message(array $task, string $event_key, int $recipient_id): array
    {
        ...
        switch ($event_key) {
            ...
            case 'task_clarification_requested':
                return [
                    'title' => 'Clarification requested',
                    'body' => '"' . $title . "\" was returned for clarification" . ($deadline ? ' before ' . $deadline : '') . '.',
                ];
            case 'task_returned_to_work':
                return [
                    'title' => 'Returned to work',
                    'body' => '"' . $title . "\" was returned to in-progress for changes.",
                ];
            case 'task_delivered':
                return [
    ...
    private static function emit_status_event(int $task_id, string $old_status, string $new_status): void
    {
        ...
        if ($new_status === 'approved') {
            self::emit_event($task_id, 'task_approved');
        } elseif ($new_status === 'needs_clarification') {
            self::emit_event($task_id, 'task_clarification_requested');
        } elseif ($new_status === 'in_progress' && in_array($old_status, ['needs_review', 'delivered'], true)) {
            self::emit_event($task_id, 'task_returned_to_work');
        } elseif ($new_status === 'delivered') {
            self::emit_event($task_id, 'task_delivered');
        } elseif ($new_status === 'archived') {
            self::emit_event($task_id, 'task_archived');
        }
    }
    ...
    private static function enrich_task_rows(array $rows): array
    {
        ...
        $preload_user_ids = [];
        foreach ($rows as $row) {
            $task_ids[] = (int) ($row['id'] ?? 0);
            $preload_user_ids[] = (int) ($row['submitter_id'] ?? 0);
            $preload_user_ids[] = (int) ($row['client_user_id'] ?? 0);
            $preload_user_ids[] = (int) ($row['action_owner_id'] ?? 0);
            $owners = json_decode((string) ($row['owner_ids'] ?? ''), true);
            if (is_array($owners)) {
                foreach ($owners as $oid) {
                    $preload_user_ids[] = (int) $oid;
                }
            }
        }
        self::preload_users($preload_user_ids);
    ...
    private static function accessible_job_ids(int $user_id): array
    {
        ...
        return self::sanitize_int_array($bucket_ids);
    }
    ...
    private static function load_invoice_draft_ledger_rows(array $entry_ids, int $user_id)
    {
        ...
        $entry_ids = self::sanitize_int_array($entry_ids);
        if (empty($entry_ids)) {
            return [];
        }
    ...
    private static function build_invoice_draft_lines_from_ledger_entries(array $rows): array
    {
        ...
        $bucket_names = [];
        $bucket_ids = self::sanitize_int_array($bucket_ids);
        if (! empty($bucket_ids)) {
        ...
        foreach ($groups as $group) {
            $task_ids = self::sanitize_int_array($group['linked_task_ids']);
            $source_snapshot = [
                'task_ids' => $task_ids,
    ...
    private static function insert_statement_line(int $statement_id, array $line, string $now): int
    {
        ...
        $linked_task_ids = self::sanitize_int_array($line['linked_task_ids'] ?? []);
    ...
    private static function statement_line_task_ids(array $line): array
    {
        ...
        return self::sanitize_int_array($task_ids);
    }
    ...
    private static function generate_statement_code(string $statement_month = ''): string
    {
        ...
        $prefix = 'STM-' . $month . '-';
        $max_code = (string) $wpdb->get_var($wpdb->prepare(
            "SELECT MAX(statement_code) FROM {$table} WHERE statement_month = %s",
            $statement_month
        ));
        $seq = 1;
        if ($max_code !== '' && str_starts_with($max_code, $prefix)) {
            $seq = ((int) substr($max_code, strlen($prefix))) + 1;
        }
    ...
    private static function generate_work_log_code(string $seed_date = ''): string
    {
        ...
        $prefix = 'LOG-' . $code_month . '-';
        $max_code = (string) $wpdb->get_var($wpdb->prepare(
            "SELECT MAX(work_log_code) FROM {$table} WHERE DATE_FORMAT(created_at, '%%Y-%%m') = %s",
            $month
        ));
        $seq = 1;
        if ($max_code !== '' && str_starts_with($max_code, $prefix)) {
    ...
    private static function normalize_task_row(array $row): array
    {
        ...
        foreach ((array) $row['owner_ids'] as $owner_id) {
            $user = self::get_cached_user((int) $owner_id);
            if ($user) {
                $row['owner_names'][] = (string) $user->display_name;
            }
        }

        $submitter = isset($row['submitter_id']) ? self::get_cached_user((int) $row['submitter_id']) : null;
        ...
        $client = $row['client_user_id'] > 0 ? self::get_cached_user($row['client_user_id']) : null;
        ...
        $action_owner = $row['action_owner_id'] > 0 ? self::get_cached_user($row['action_owner_id']) : null;
        ...
        $row['action_owner_is_client'] = $row['action_owner_id'] > 0 && $row['client_id'] > 0
            ? ! empty(array_filter(self::get_cached_client_memberships((int) $row['client_id']), static function (array $membership) use ($row): bool {
